; ATCACHE TODOs:
;	1) Process needs to use basicAt:/basicAtPut: primitives (use basicAt from the stack frame itself too).
;		Do we need to flush process ote from at cache then? No, because it can't get in there.
;	2) I think atPutCached primitive will crash on overflow due to not popping pushed edi and ebx
;	3) StringAtPut needs to update cache, and need to implement cached at put for strings in byteasm.asm
;	4) Think about whether there are any other places that might need to flush at cache (resizing, changing object address, etc)
;	5) Would it make sense to flush at cache on reallocating pointer rather than when deallocating (would it make any difference)
;
; IDEAS:
;	1: AtCache from Squeak. Holds most recently #at:'d objects that had primitive response.
;		Only problem is need to remove entries when objects deleted and on GC (put in method cache flush)
;	2: Increment often succeeds a push of temporary, e.g. in the optimized version of #to:do: generated by the compiler,
